...
namespace {
	TEST(AlignmentLaw,BasicsTests) {
        ...
        EXPECT_EQ(alignments[1].y, 1.f);
        EXPECT_EQ(alignments[2].x, 0.5f);
        EXPECT_EQ(alignments[2].y, 0.f);
	}
    ...
    TEST(CohesionLaw, BasicTests) {
        ...
        EXPECT_NEAR(cohesions[2].x, 30.f, 1); //0 because out of range from [1] (60 > 50) and angle from [0] (135,001 > 135 or 135 !< 270/2)
        EXPECT_NEAR(cohesions[2].y, -30.f, 1);
    }
    ...
    TEST(SeparationLaw, BasicTests) {
        ...
        EXPECT_NEAR(separations[1].x, 1.f, 0.1f);
        EXPECT_NEAR(separations[1].y, 1.f, 0.1f);
        EXPECT_NEAR(separations[2].x, -2.f, 0.1f);  //-1.f and not -2.f because a neighbor is not a viable neighbor
        EXPECT_NEAR(separations[2].y, 1.f, 0.1f);   //0.f and not -1.f because a neighbor is not a viable neighbor
    }
}