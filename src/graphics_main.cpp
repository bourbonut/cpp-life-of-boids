...
//    ShaderProgram_activate(triangle_shaderProgram); // Premire apparition de l'erreur
...
//
//    std::vector<points::Point> points(10);
//    auto get_pos = [=](float t) {
...
//
//        {  // lines
//            mat3x3 transform = points::vertex_transform_2d((float)width, (float)height);
//
//            VertexArray_bind(lines_vertexArray);
//            Buffer_bind(lines_buffer, GL_ARRAY_BUFFER);
//            ShaderProgram_activate(lines_shaderProgram);
...
//
//            long int N = 100000;
//            std::vector<mat2x2> triangles{};
//            for (int i = 0; i < N; ++i) {
//                triangles.push_back(mat2x2{ randomVec2Generation(0, 800), randomVec2Generation(-5, 5) });
//            }
//
//            
//            float v = 0;
//            int i{};
//            bool prettyAgents{false};
...
//                        mat2x6 result = drawAgent(triangles[k][0], triangles[k][1]);
//
//                        for (long int j = 0; j < result.size(); ++j) {
//                            vertex_data.push_back(triangle::Vertex{ {result[j].x, result[j].y}, {0.0, 1.0, 1.0} }); // Vertex A |
//                        }
//                    }
//                    else {
//                        // Fill vertex array of points for each agents
//                        Vec2 res = triangles[k][0];
//                        vertex_data_dots.push_back(points::Vertex{ {res.x, res.y}, {1.f, 1.f, 1.f} });
//
//                    }
//                }
//
...
//                    glUniform1f(pointSize_location, 3.f);
//                    glUniform1f(maxSpeedSquared_location, 10.f);
...
//                std::cout << "Pour i = " << i << " elapsed seconds = " << elapsed_seconds.count() << std::endl;
//            }
//
//
//        }
