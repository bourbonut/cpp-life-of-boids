...
Bird::Bird(const Vec2& position, const Vec2& velocity) : m_position{ position }, m_velocity{ velocity }, m_bodySize{ 1 } {};

void Bird::move() {
	m_position += m_velocity;
};

void Bird::setPosition(const Vec2& newPos) {
	m_position = newPos;
};

void Bird::computeLaws(const std::vector<Bird>& neighbors) {
	...
	m_velocity = vec_displacement.normalize() * 0.5;
};
