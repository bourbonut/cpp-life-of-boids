...
std::vector<Bird> Flock::computeNeighbors(const Bird& bird, const float& range, const float& angle) {
	...
	for (Bird potentialNeighbor : m_birdsVec) {
		//std::cout << degrees(bird.getVelocity().angle()) << '\n';
		//std::cout << bird.getPosition() - potentialNeighbor.getPosition() << '\n';
		//std::cout << degrees((bird.getPosition() - potentialNeighbor.getPosition()).angle()) << '\n';
		//if (degrees((bird.getPosition() - potentialNeighbor.getPosition()).angle()) > angle / 2)
		//{
		//	std::cout << degrees((bird.getPosition() - potentialNeighbor.getPosition()).angle()) << '\n';
		//}
		//std::cout << "id:" << bird.getId() << "   pos:" << bird.getPosition() << '\n';
		//std::cout << "id:" << potentialNeighbor.getId() << "   pos:" << potentialNeighbor.getPosition() << '\n';
